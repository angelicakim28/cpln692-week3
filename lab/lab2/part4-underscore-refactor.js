(function(){
  ...
  // }
    ...
    _.each(filtered_data, function(filteredData) {
      isOpen = filteredData.ACTIVE.toUpperCase() == 'OPEN';
      isPublic = (filteredData.TYPE.toUpperCase() !== 'CHARTER' ||
                  filteredData.TYPE.toUpperCase() !== 'PRIVATE');
      meetsMinimumEnrollment = filteredData.ENROLLMENT > minEnrollment;

      // Constructing the styling  options for our map
      if (filteredData.HAS_HIGH_SCHOOL){
        color = '#0000FF';
      } else if (filteredData.HAS_MIDDLE_SCHOOL) {
        color = '#00FF00';
      } else {
        color = '##FF0000';
      }
      // The style options
      var pathOpts = {'radius': filteredData.ENROLLMENT / 30,
                      'fillColor': color};
      L.circleMarker([filteredData.Y, filteredData.X], pathOpts)
        .bindPopup(filteredData.FACILNAME_LABEL)
        .addTo(map);
    })
